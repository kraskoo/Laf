namespace LafAPI.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> Friends([FromQuery] string search = null, [FromQuery] bool friends = false)
        {
            ...
            return this.Ok(
                !string.IsNullOrEmpty(search) ? this.UserServices.UsersWithoutFriendshipWithCurrentUser(
                            id,
                            u => u.FirstName.ToLower().Contains(search) || u.LastName.ToLower().Contains(search) ||
                                 u.Email.ToLower().Contains(search))
                        .To<UserViewModel>() :
                    this.UserServices.UsersWithoutFriendshipWithCurrentUser(id).To<UserViewModel>());
        }

        [HttpGet]
        [Route("uncofirmedfriendships")]
        public async Task<IActionResult> GetUncofirmedFriendships() =>
            this.Ok(await this.userFriendService.GetByFriendIdAndStatus(this.User.GetId(), FriendshipStatusType.Invited));
        ...
        public async Task<IActionResult> Register([FromBody] UserRegisterBindingModel model)
        {
            ...
            var role = !await this.UserServices.AnyAsync() ? nameof(RoleType.Administrator) : nameof(RoleType.Regular);
            ...
            var response = this.UserServices.Authenticate(model.Email, model.Password, this.HttpContext, out user);
            if (response.Result == UserService.InvalidEmail)
            {
                this.ModelState.AddModelError(nameof(UserService.InvalidEmail), UserService.InvalidEmail);
                return this.BadRequest(this.ModelState);
